...
def utc_string(dt):
    """
    Convert datetime to UTC ISO-8601 format expected by Eka.
    Example:
...
def create_consent(
    ...
    """
    Create a consent request using Eka API.

    POST /abdm/v1/consents/create

    This creates the consent REQUEST.

    It does NOT mean the patient has approved the consent.
    ...
    #
    # Screenshot:
    # Shared records expire in 6 months
    # --------------------------------------------------------

    expiry = (
        now + timedelta(
            days=30 * expiry_months
        )
    )
    ...
    payload = {
        ...
        "period": {
            ...
            "from":
                utc_string(period_from),

            "to":
                utc_string(period_to),
        },
    ...
    try:
        ...
        print(
            "CONSENT CREATE REQUEST FAILED:"
        )

        print(exc)

        raise RuntimeError(
            "Unable to call Eka Consent Create API."
        ) from exc
    ...
    if not consent_init_id:
        ...
        raise RuntimeError(
            "Consent Create succeeded but "
            "consent_init_id was not returned."
        )
...
@app.post(
    "/api/consent/create/<care_context_id>"
)
