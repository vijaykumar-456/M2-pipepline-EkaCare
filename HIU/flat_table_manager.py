...
# def _excel_wrap(value: str) -> str:
#     """Wrap a value as an Excel text-formula so long digit strings
#     (patient IDs, OIDs, mobile numbers) display exactly as stored when
#     the raw CSV is opened directly in Excel -- no "convert to number?"
#     prompt, no precision loss. Empty values are left alone."""
#     if not value:
#         return value
...
#
#
# def _excel_unwrap(value: str) -> str:
#     """Reverse of _excel_wrap. Safe to call on values that were never
#     wrapped (old rows written before this change) -- returns them
#     unchanged."""
...
#
#         for r in csv.DictReader(f):
#             for col in _TEXT_SAFE_COLUMNS:
#                 if col in r and r[col]:
#                     r[col] = _excel_unwrap(r[col])
#
#             record_id = r.get("record_id", "").strip()
#
#             if not record_id:
...
def generate_care_context(
    ...
    for existing_record_id, existing in rows.items():
        if (
                existing.get("row_id") == row_id
                and not existing.get("encounter_id")
                and not existing.get("care_context_id")
        ):
            record_id = existing_record_id
            break

    # If no identity-only record exists, create a new record.
